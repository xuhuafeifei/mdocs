...
// 换成你要测试的文档或目录 ID
const TEST_DOC_ID = '5d46d3b7-ccfd-4822-b6a3-d1c96810b5ac';
...
describe('buildGraphByDocId 端到端测试', () => {
  it('可以正常构建图谱', async () => {
    ...
    console.log(`  doc 节点：${graph.nodes.filter(n => n.type === 'doc').length}`);
    console.log(`  concept 节点：${graph.nodes.filter(n => n.type === 'concept').length}`);

    // 打印前 3 个节点看看
    console.log('\n前 3 个节点：');
    for (const node of graph.nodes.slice(0, 3)) {
      console.log(`  [${node.type}] ${node.label} — ${node.description.slice(0, 50)}...`);
    }
    ...
    if (graph.edges.length > 0) {
      ...
      for (const edge of graph.edges.slice(0, 3)) {
        console.log(`  ${edge.from} → ${edge.to} (${edge.type}, ${edge.confidence})`);
      }
    }
