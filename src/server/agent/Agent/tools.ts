...
export function mdocsManualOutlineTool(deps: ToolDeps): AgentTool {
  const { skills } = deps;
  if (!skills) throw new Error("mdocs_manual_outline requires skills");
  return {
...
export function mdocsManualContentTool(deps: ToolDeps): AgentTool {
  const { skills } = deps;
  if (!skills) throw new Error("mdocs_manual_content requires skills");
  return {
    ...
    execute: async (_id, params) => {
      ...
      if (!body) {
        throw new Error(`手册章节不存在: ${id}，请先调用「查询 mdocs 手册大纲」`);
      }
