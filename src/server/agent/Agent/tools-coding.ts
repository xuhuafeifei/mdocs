...
function clipMarkdown(md: string): {
  text: string;
  truncated: boolean;
  charCount: number;
} {
...
function requireCoding(deps: ToolDeps): CodingDocContext {
  if (!deps.coding) throw new Error("coding tools require coding ctx");
  return deps.coding;
}

/** coding：读前端 MD buffer，不写磁盘 */
export function getWorkingDocumentTool(deps: ToolDeps): AgentTool {
  const ctx = requireCoding(deps);
  return {
    ...
    parameters: Type.Object({
      which: Type.Optional(
        Type.Union(
          [Type.Literal("working"), Type.Literal("base"), Type.Literal("both")],
...
/** coding：写前端提案正文，不写磁盘 */
export function setMarkdownDocumentTool(deps: ToolDeps): AgentTool {
  requireCoding(deps);
  return {
