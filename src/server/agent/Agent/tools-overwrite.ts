...
function classifyChoice(choice: string): "overwrite" | "coding" | "cancel" {
  ...
  // 自由输入：含「覆写」优先；否则不当作同意覆写
  if (t.includes("覆写")) return "overwrite";
  return "cancel";
}

export type OverwriteToolHooks = ChoiceToolHooks & {
 ...
 */
export function createOverwriteTools(
  visitorId: string,
  hooks: OverwriteToolHooks,
): AgentTool[] {
  return [
    {
      ...
      execute: async (_id, params) => {
        ...
        try {
          ...
          if (action === "coding") {
            hooks.onOpenCoding({ documentId, displayName });
            return asToolResult({
