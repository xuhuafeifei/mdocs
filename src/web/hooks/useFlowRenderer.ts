...
import {
  findMeta2BlockLineNumber,
  findMeta2BlockRange,
} from "../diagram/meta2Markdown";
...
export function useFlowRenderer(
  ...
  onEdit: (lineNumber: number, flowData: any, blockIndex: number, rawJson: string) => void,
  readOnly = false,
) {
  ...
  useEffect(() => {
    ...
    const openEditorForBlock = (block: Element) => {
      ...
      try {
        ...
        onEditRef.current(targetLine, flowData, blockIndex, raw);
      } catch (err) {
